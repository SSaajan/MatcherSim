self.onmessage = async function(e) {
    ...
    //Defining an arbitrary constant running time for processes
    const RUNNING_TIME = e.data.RUNNING_TIME;

    //Reading and parsing input files
    try {
        const processFileContent = await readFileAsText(e.data.processFile);
        processData = parseProcessCsv(processFileContent);
        const resourceFileContent = await readFileAsText(e.data.resourceFile);
        resourceData = parseResourceCsv(resourceFileContent);
    } catch (error) {
        console.error('Error reading files:', error);
    }

    console.log('Process Rows:', processData);
    console.log('Resource Rows:', resourceData);

    const resultColumns = ["process_id", "resource_id", "matchedAt", "wait_time_secs", "status"];
    //Running the matching algorithm
    const simResult = matchResourceToProcess(resourceData, processData, RUNNING_TIME);

    // Convert simResult object to CSV array
    let resultString = resultColumns.join(",") + "\n";
    for(const processId in simResult) {
        const process = simResult[processId];
        const row = resultColumns.map(column => {
            if(process[column] === undefined) {
                return "-"; // Use "-" for missing values
            } else {
    ...
    console.log('FINAL: ', simResult);
    console.log('CSV Result:', resultString);
    self.postMessage({
...
function parseResourceCsv(csv) {
    const lines = csv.split('\n');
    const data = lines.map(line => {
        const [resource_id, available_at_str, capabilities_str] = parseCSVLine(line);
        let capabilities = {};
        try {
            capabilities = JSON.parse(capabilities_str);
        } catch (err) {
            console.error('Failed to parse JSON:', line);
            console.error('Failed to parse JSON:', capabilities_str);
        }
        return {
            resource_id,
            available_at: Math.floor(new Date(available_at_str).getTime() / 1000),
            capabilities
        };
    });
    return data;
...
function parseProcessCsv(csv) {
    const lines = csv.split('\n');
    const data = lines.map(line => {
        const [process_id, request_time_str, timeout_str, constraints_str, preferences_str] = parseCSVLine(line);;
        let constraints = {};
        let preferences = {};
        try {
            constraints = JSON.parse(constraints_str);
            preferences = JSON.parse(preferences_str);
        } catch (err) {
            console.error('Failed to parse JSON:', capabilities_str);
        }
        return {
            process_id,
            request_time: Math.floor(new Date(request_time_str).getTime() / 1000),
            constraints,
            preferences,
...
function matchResourceToProcess(resourceData, processData) {
    const runningTime = 2;
    ...
    processQueue = processData.map(process => process.process_id).sort((a, b) => a.request_time - b.request_time);
